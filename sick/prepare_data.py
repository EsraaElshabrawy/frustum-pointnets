...
import sick_util as utils
import pickle
from sick_object import *
import argparse
...
def demo():
    ...
    dataset = sick_object(os.path.join(ROOT_DIR, 'dataset/SICK/object'))
    data_idx =40
    ...
    # imgfov_pts_2d.shape[0]
    # draw_lidar(imgfov_pts_2d, fig=fig)
...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    '''
    dataset = sick_object(os.path.join(ROOT_DIR,'dataset/SICK/object'), split)
...
def get_box3d_dim_statistics(idx_filename):
    ...
    dataset = kitti_object(os.path.join(ROOT_DIR,'dataset/SICK/object'))
    dimension_list = []
    type_list = []
    ry_list = []
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)]
...
def read_det_file(det_filename):
    ...
    det_id2str = {0:'Pedestrian', 2:'Car', 3:'Cyclist'}
    id_list = []
    type_list = []
    prob_list = []
    box2d_list = []
    for line in open(det_filename, 'r'):
        t = line.rstrip().split(" ")
        print((os.path.basename(t[0]).rstrip('.png')))
...
def extract_frustum_data_rgb_detection(det_filename, split, output_filename,
    ...
    for det_idx in range(len(det_id_list)):
        ...
        if ymax-ymin<img_height_threshold or \
            ...
            #print(ymax,ymin,ymax-ymin,len(pc_in_box_fov))
            continue
...
def write_2d_rgb_detection(det_filename, split, result_dir):
    ...
    for idx in results:
        pred_filename = os.path.join(output_dir, '%02d.txt'%(idx))
