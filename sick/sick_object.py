...
class sick_object(object):
    ...
    def __init__(self, root_dir, split='testing'):
        ...
        self.lidar_dir = os.path.join(self.split_dir, 'pointcloud')
    ...
    def get_image(self, idx):
        ...
        img_filename = os.path.join(self.image_dir, '%02d.png'%(idx))
        return utils.load_image(img_filename)

    def get_lidar(self, idx): 
        assert(idx<self.num_samples) 
        lidar_filename = os.path.join(self.lidar_dir, '%02d.bin'%(idx))
        return utils.load_velo_scan(lidar_filename)

    def get_calibration(self, idx):
        assert(idx<self.num_samples) 
        calib_filename = os.path.join(self.calib_dir, '%02d.txt'%(idx))
